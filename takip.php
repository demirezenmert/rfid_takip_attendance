...
//Check if we have a month passed in through a get variable, otherwise use the current yea
r
if (isset($_GET['month'])) {
...
<html lang="en">
    ...
    <nav class="navbar navbar-dark bg-dark">
        ...
        <ul class="nav nav-pills">
            <li class="nav-item">
                <a href="attendance.php" class="nav-link active">Takip Çizelgesi</a>
            </li>
            <li class="nav-item">
                <a href="users.php" class="nav-link">Kullanıcılar</a>
            </li>
        </ul>
    </nav>
    <div class="container">
                ...
                <tr>
                    ...
                    <?php
                        ...
                        for ( $iter = 1; $iter <= $num_days; $iter++) {
                            echo '<th scope="col" style="min-width:200px;max-width:300px;"
>' . $iter . '</th>';
                        }
                    ?>
                </tr>
                ...
                <?php
                    ...
                    foreach($users as $user) {
                        ...
                        for ( $iter = 1; $iter <= $num_days; $iter++) {
                            ...
                            //For each pass grab any attendance that this particular user 
might of had for that day
                            $attendance = $database->select("attendance", [
                                'clock_in'
                            ], [
                                'user_id' => $user['id'],
                                'clock_in[<>]' => [
                                    date('Y-m-d', mktime(0, 0, 0, $current_month, $iter, $
current_year)),
                                    date('Y-m-d', mktime(24, 60, 60, $current_month, $iter
, $current_year))
                                ]
                            ]);
                            ...
                            if(!empty($attendance)) {
                                //If we have found some data we loop through that adding i
t to the tables cell
                                ...
                                //If there was nothing in the database notify the user of 
this.
